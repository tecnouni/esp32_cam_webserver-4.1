/*
ESP32_Cam_CameraWebServer-4.1.ino
descargada de https://github.com/easytarget/esp32-cam-webserver 
Modificado por mi ESP32_Cam_CameraWebServer-4.0.ino

//Pongo directamente los pines de CAMERA_MODEL_AI_THINKER
Quito cosas
Falta encender led y control servos
...
char baseVersion[] = "ESP32_Cam_CameraWebServer-4.1.ino";// Versión de la aplicación
//Configuro servos
#define SERVO_1      14
#define SERVO_2      15
#define SERVO_STEP   5
...
//Pongo los pines directamente  de CAMERA_MODEL_AI_THINKER  obtenidos de #include "camera_pins.h"
...
// Illumination LAMP and status LED
    ...
    int lampVal = -1; // lamp is disabled in config
        ...
        int lampVal = constrain(LAMP_DEFAULT,0,100); // initial lamp value, range 0-100 Restringe un número para que esté dentro de un rango.
    #else
...
void moverservo(int nservo,int pos) {
  /*if(servo2Pos >= 10) {
    ...
    if (nservo == 1) {servo1.write(pos);}
    if (nservo == 2) {servo2.write(pos);}
    Serial.print(nservo);
    Serial.print("pos: ");
    Serial.println(pos);
}
...
void setup() {
    ...
    Serial.println();
    Serial.println("====");
    Serial.print("ESP32_Cam_CameraWebServer-4.1.ino: ");
...
///***************cambio*****************************
  ...
  servo1.attach(SERVO_1, 1000, 3000);
  servo2.attach(SERVO_2, 1000, 3000);
...
void loop() {
/*
* Simplemente repetir el bucle para siempre, reconectando el Wifi según sea necesario en el modo cliente
* Los procesos de transmisión y controlador de URI iniciados por la llamada startCameraServer() al
* final de setup() manejarán el procesamiento de la cámara y de la interfaz de usuario a partir de ahora.
*/
    if (accesspoint) {
      // El punto de acceso está activo de forma permanente, por lo que solo hay que repetirlo, prestando servicio al portal cautivo según sea necesario. 
      // En lugar de repetirlo eternamente, hay que seguir al perro guardián, en caso de que más adelante agreguemos un nuevo escaneo automático.
        unsigned long start = millis();
        ...
        // El modo cliente puede fallar, por lo que debe volver a conectarse según corresponda
        static bool warned = false;
        if (WiFi.status() == WL_CONNECTED) {
            //Estamos conectados, espera un poco y vuelve a verificar
            if (warned) {
                // Decirle al usuario si acabamos de reconectarnos
                Serial.println("WiFi reconnected ESP32_Cam_CameraWebServer-4.1.ino");
                warned = false;
            }
            // bucle aquí para WIFI_WATCHDOG, convirtiendo debugData en verdadero/falso dependiendo de la entrada serial.
            unsigned long start = millis();
            while (millis() - start < WIFI_WATCHDOG ) {
                delay(100);
                if (otaEnabled) ArduinoOTA.handle();
            ...
            // desconectado; intentar reconectarse
            if (!warned) {
                //Dígale al usuario si acabamos de desconectarnos
                WiFi.disconnect();  // asegura que la desconexión esté completa y que el escaneo wifi esté borrado
